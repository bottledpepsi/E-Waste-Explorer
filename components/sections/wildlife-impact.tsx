...
const CARDS = [
  ...
  },
  {
    ...
    text: 'Poisons build up as they move from small creatures to bigger animals — and can even end up in the food people eat.',
  },
  {
    Icon: Sprout,
    title: 'Habitats',
    ...
    <section id="wildlife-impact" className="bg-background py-20 sm:py-24">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <Reveal className="mx-auto max-w-3xl text-center">
          <span className="mx-auto grid size-14 place-items-center rounded-2xl bg-brand-teal/15 text-brand-teal">
          ...
          <p className="mt-4 text-lg leading-relaxed text-pretty text-foreground/80 sm:text-xl">
            Wildlife cannot choose where to live, so pollution from e-waste hits
            them hard.
          </p>
        </Reveal>
        ...
        <ul className="mt-12 grid gap-6 md:grid-cols-3">
          {CARDS.map(({ Icon, title, text }, i) => (
            <Reveal as="li" key={title} delay={i * 80}>
              <div className="flex h-full flex-col gap-4 rounded-3xl border border-brand-teal/15 bg-brand-teal-soft p-6 shadow-sm transition-transform hover:-translate-y-1">

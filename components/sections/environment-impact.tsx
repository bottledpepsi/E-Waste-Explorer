import {Droplets, Factory, Wind} from 'lucide-react'
...
const CARDS = [
    ...
    },
    {
        Icon: Wind,
        title: 'Air',
        text: 'Burning e-waste to get the metals out (common in unsafe recycling) releases poisonous fumes and smoke into the air.',
    },
    {
        Icon: Factory,
        title: 'Wasted Resources',
        text: 'When we do not recycle, we have to mine new gold, copper and rare metals from the Earth, which damages it.',
    },
]
                ...
                <ul className="mt-12 grid gap-6 md:grid-cols-3">
                    {CARDS.map(({Icon, title, text}, i) => (
                        <Reveal as="li" key={title} delay={i * 80}>
                            <div
                                className="flex h-full flex-col gap-4 rounded-3xl border border-brand-green/15 bg-card p-6 shadow-sm transition-transform hover:-translate-y-1">
